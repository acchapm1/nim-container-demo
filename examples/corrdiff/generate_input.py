...
GEFS_VARIABLES = ["u1000", "u925", "u850", "u700", "u500", "u250",
...
def fetch_input_gefs(time: datetime, lead_time: timedelta, content_dtype: str = "float32"):
    ...
    pressure_data = torch.nn.functional.interpolate(
        torch.Tensor(pressure_data.values), (len(GEFS_VARIABLES), 721, 1440), mode="nearest"
    ).numpy()[:, 0, :, 148:277, 900:1201].astype(dtype)
